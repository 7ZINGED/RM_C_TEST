...
//遥控器的控制信息转化为标准单位，平移为(mm/s)旋转为(degree/s)
    ...
    //尽量实现DT7状态机的全部功能
    //优先考虑失能状态设计
    //sw3为上时，云台底盘均RELAX,为中云台为GYRO,为下时为AUTO
    //SW2为上时底盘为FOLLOW,为中时为open，为下时为SPIN
    switch (rc_now->sw2)
    {
        case RC_UP:
        ...
        case RC_DN:
            ...
            chassis_cmd.vw = 1; // 小陀螺转速(我也不确定给多少)
            break;
    }
    //若sw3上拨，则应无视sw2的状态，故应后判断
    switch(rc_now->sw3)
    {
        case RC_UP:
    ...
    switch(rc_now->sw1)
    {
        case RC_DN:
        {
            if(rc_now->ch5 <= 783.0f)
            {
                trigger_flag = 1;
            }
            if(trigger_flag&&rc_now->ch5>=700)
            {
...
//遥控器的控制信息转化为标准单位，平移为(mm/s)旋转为(degree/s)
    /*底盘命令*/
    //这里好像是忘了改成subs？,但由于相同情况下速度应一致，此处宏定义不应改动
    ...
    //若扳机开火，且为发射状态
    if((shoot_cmd.trigger_status == TRIGGER_ON)&&(shoot_cmd.ctrl_mode!=SHOOT_STOP)&&(shoot_cmd.ctrl_mode!=SHOOT_REVERSE))
    {
